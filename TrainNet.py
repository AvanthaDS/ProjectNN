...
class tRainNet:
    ...
    def tRain(self):
        ads_neurons_n = self.nn  # 10  # number of Neurons required - Fix
        ads_lrn_rate = self.ln_rate  # 1  # enter learning rate - Fix
        ads_itn_n = 4  # number of iterations required
        ads_itn_cntr = 1  # do not change manually
        ads_bias = self.bias  # 1
        ads_tgt_error = self.tgt_er  # 0.01
        ads_function = self.fn  # 1  # 1 - Sigmoid , 2 - Hypabolic tangent
        ads_in_ws = []
        ads_out_ws = []
        ads_numberof_itn = 1
        ...
        if len(t_in_lst) != len(t_out_lst):
            ...
            while ads_itn_cntr >= ads_tgt_error:
                ...
                self.ads_abs_error.append(sum(ads_temp_err2))

                del ads_temp_err2[:]

                ads_numberof_itn += 1
                ads_itn_cntr = self.ads_abs_error[-1]
        ...
        start_err = self.ads_abs_error[0]
